...
Console.WriteLine(giftCard.GetAccountStatement());
...
Console.WriteLine(savings.GetAccountStatement());
...
Console.WriteLine(lineOfCredit.GetAccountStatement());

// * Comment the follwing code to stop running the test
...
try{
    lineOfCredit.MakeWithdrawal(4000m, DateTime.Now, "repay");
}
catch (InvalidOperationException e){
    Console.WriteLine("Exception caught trying to overdraw");
    Console.WriteLine(e.ToString());
